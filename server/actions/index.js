...
const deleteProduct = async query => {
  return await global.databaseInstance
    .collection(config.collections.product)
    .removeOne(query, true);
};
...
const updateRequest = async (reqId, status) => {
  ...
  return await global.databaseInstance
    ...
    .updateOne({ reqId: reqId }, { $set: { status: status } });
};
